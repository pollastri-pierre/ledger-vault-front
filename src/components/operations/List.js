              ...
              <tr>
                {_.map(columnIds, (column, i) => {
                  return [
                    ...
                    <td
                      className={i === nbColumn - 1 ? "align-right" : ""}
                      key={i}
                    >
                      {column}
                    </td>
            ...
            <tbody>
              {_.map(operations, operation => {
                ...
                return (
                  <tr
                    key={operation.uuid}
                    onClick={e => props.open(operation.uuid)}
                  >
                    {_.map(columnIds, (column, i) => {
                      switch (column) {
                        case "date":
                          return (
                            <td className="date" key={i}>
                              ...
                              {operation.notes.length > 0 ? (
                                <Tooltip
                                  id={`tooltip-${operation.uuid}`}
                                  className="tooltip-label"
                                >
                                  <p className="tooltip-label-title">
                                    {note.title}
                                  </p>
                                  ...
                                  <div className="hr" />
                                  <p className="tooltip-label-body">
                                    {note.body}
                                  </p>
                                </Tooltip>
                              ) : (
                          ...
                          return (
                            <td key={i}>
                              <div>
                                <BadgeCurrency
                                  currency={
                                    accounts[operation.account_id].currency
                                  }
                                />
                                <span className="uppercase currencyName">
                                  {accounts[operation.account_id].name}
                                </span>
                              </div>
                            </td>
                          );
                        ...
                        case "amount":
                          return [
                            <td className="amount-euro" key={i}>
                              <CurrencyNameValue
                                currencyName={
                                  operation.reference_conversion.currency_name
                                }
                            ...
                            <td
                              key={nbColumn}
                              className={`amount-crypto ${operation.amount > 0
                                ? "positive"
                                : ""}`}
                            >
                              <CurrencyNameValue
                                currencyName={
                                  accounts[operation.account_id].currency.name
                                }
                        ...
                        default:
                          throw new Error(
                            `Unsupported column name "${column}"`
                          );
                      }
                    })}
                  </tr>
