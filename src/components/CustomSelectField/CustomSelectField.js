  ...
  render() {
    ...
    return (
      <div className="CustomSelectField">
        ...
        <div className="LabelSelectField" ref={(el) => this.filter = el} onClick={() => this.toggle()}>
        ...
        >
          {
            _.map(values, value => (
              <div className={`menuElem ${selected.title === value.title ? "selected" : ""}`} onClick={()=> {this.close(); this.onChange(value.key)}}>
                {value.title}
              </div>
            ))
          }
