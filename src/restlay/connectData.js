...
>(Decorated: In<Props, A>, opts?: Opts<Props, A>): Out<Props> {
    ...
    class Clazz extends Component<ClazzProps<Props>, State> {
            ...
            const promises: Array<Promise<*>> = queriesKeys
                .map(key => {
                    ...
                    if (query instanceof ConnectionQuery) {
                        ...
                        if (!needsRefresh) {
                            const cache = getPendingQueryResult(
                                dataStore,
                                query
                            );
                            needsRefresh =
        ...
        syncProps(
            ...
            if (promises.length > 0) {
                ...
                p = Promise.all(promises).then(
                    () => {
                        if (this._unmounted || syncId !== this.syncAPI_id)
                            return;
        ...
        restlay: RestlayEnvironment = {
            ...
            forceFetch: () =>
                this.syncProps(this.props, {}, true).then(() => {}),
        ...
        render() {
            ...
            if (error) {
                // there is an error
                return (
                    <RenderError {...props} error={error} restlay={restlay} />
                );
            }
            if (
