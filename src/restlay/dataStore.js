...
const accumulateConnectionEdges = <T>(
    ...
    if (process.env.NODE_ENV === "development" && oldConnection) {
        ...
        edges.forEach((e, i) => {
            ...
            if (cursors[e.cursor]) {
                console.warn(
                    "API issue: duplicate cursor found at index " +
                        i +
                        ": " +
                        e.cursor
                );
        ...
        if (queryOrMutation instanceof ConnectionQuery) {
            ...
            if (count < 0) {
                ...
                return Promise.resolve().then(() => {
                    ...
                    return cache
                        ? sliceConnection(cache.result, size)
                        : emptyConnection;
                });
            } else if (count === 0) {
                // $FlowFixMe
                return Promise.resolve(
                    (cache && cache.result) || emptyConnection
                );
            } else {
                const params = queryOrMutation.getPaginationURLParams(
                    ...
                    shouldReset || !cache || cache.result.edges.length === 0
                        ? undefined
                        : cache.result.edges[cache.result.edges.length - 1]
                              .cursor
                );
