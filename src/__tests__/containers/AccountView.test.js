...
describe('AccountView container', () => {
  it('should call update when mounting', () => {
    const spy = jest.spyOn(AccountViewNotDecorated.prototype, 'update').mockImplementation(() => {});
  ...
  it('infos-left-top component should contain BalanceCard and CounterValueCard', () => {
    ...
    expect(infos.find('BalanceCard').prop('data')).toEqual(props.accountsInfo.balance);
    expect(infos.find('BalanceCard').prop('loading')).toEqual(props.accountsInfo.isLoadingBalance);

    expect(infos.find('CounterValueCard').length).toBe(1);
    expect(infos.find('CounterValueCard').prop('data')).toEqual(props.accountsInfo.countervalue);
    expect(infos.find('CounterValueCard').prop('loading')).toEqual(props.accountsInfo.isLoadingCounter);
  });

  it('infos-left component should contain ReceiveFundsCard component', () => {
    const wrapper = shallow(<AccountViewNotDecorated {...props} />);
    const infos = wrapper.find('.infos-left');
    expect(infos.find('ReceiveFundsCard').prop('data')).toEqual(props.accountsInfo.receiveAddress);
    expect(infos.find('ReceiveFundsCard').prop('loading')).toEqual(props.accountsInfo.isLoadingAddress);
  ...
  it('account-view-infos component should contain Quicklook component', () => {
    ...
    expect(infos.find('Quicklook').prop('operations')).toEqual(props.accountsInfo.operations);
    expect(infos.find('Quicklook').prop('loading')).toEqual(props.accountsInfo.isLoadingOperations || props.accountsInfo.isLoadingNextOperations);
  ...
  it('account-view component should contain ListOperation component', () => {
    ...
    expect(infos.find('List').prop('operations')).toEqual(props.accountsInfo.operations);
    expect(infos.find('List').prop('loading')).toEqual(props.accountsInfo.isLoadingOperations);
