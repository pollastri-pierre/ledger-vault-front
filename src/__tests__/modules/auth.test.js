...
describe('Module auth', () => {
  ...
  it('should handle failure of start_authentication', (done) => {
    ...
    moxios.wait(() => {
      ...
      request.respondWith({
        ...
        const calls = dispatch.mock.calls;
        expect(calls[0][0]).toEqual({type: START_AUTHENTICATION});
        expect(calls[1][0]).toEqual({type: CHECK_TEAM_ERROR});
        done();
      });
    });
  });
  ...
  it('reducer should set the field value', () => {
    ...
    const stateReduced = {isAuthenticated: true, team: 'test' };
  ...
  it('reducer should set the isCheckingTeam to true', () => {
    ...
    const stateReduced = {isAuthenticated: true, team: '', isCheckingTeam: true};

    expect(reducer(state, action)).toEqual(stateReduced);
  });

  it('reducer should set the teamError to true and isCheckingTeam to false', () => {
    const state = {isAuthenticated: true, team: '', teamError: false, isCheckingTeam: true };
    const action = {type: CHECK_TEAM_ERROR};
    const stateReduced = {isAuthenticated: true, team: '', teamError: true, isCheckingTeam: false};

    expect(reducer(state, action)).toEqual(stateReduced);
  });

  it('reducer should set the errorTeam to false', () => {
    const state = {isAuthenticated: true, team: '', teamError: true, isCheckingTeam: false };
    const action = {type: REMOVE_TEAM_ERROR};
    const stateReduced = {isAuthenticated: true, team: '', teamError: false, isCheckingTeam: false};

    expect(reducer(state, action)).toEqual(stateReduced);
  });

  it('reducer should set the teamError to false and isCheckingTeam to false, and set the user', () => {
    const state = {isAuthenticated: true, team: '', teamError: true, isCheckingTeam: true};
    const action = {type: CHECK_TEAM_SUCCESS};
    const stateReduced = {isAuthenticated: true, teamValidated: true, team: '',  teamError: false, isCheckingTeam: false};
