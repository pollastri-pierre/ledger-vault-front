...
describe("AccountApproveDetails tab component", () => {
  describe("Security items", () => {
    ...
    it("first .confirmation-security-item should be the number of members", () => {
      ...
      expect(
        wrapper
          .find(".confirmation-security-item")
          .at(0)
          .find(".security-title")
          .text()
      ).toBe("Members");
  ...
  describe("Account informations", () => {
    ...
    it("first .confirmation-info should be the status", () => {
      ...
      expect(
        wrapper
          .find(".confirmation-infos .confirmation-info")
          .at(0)
          .find(".info-title")
          .text()
      ).toBe("Status");
      expect(
        wrapper
          .find(".confirmation-infos .confirmation-info")
          .at(0)
          .find(".info-value.status")
          .text()
      ).toBe("Collecting approvals (33%)");
    });
    ...
    it("second .confirmation-info should be the creation time", () => {
      ...
      expect(
        wrapper
          .find(".confirmation-infos .confirmation-info")
          .at(1)
          .find(".info-value.date")
          .text()
      ).toBe(formatDate(props.account.creation_time, "lll"));
    });
    ...
    it("third .confirmation-info should be the name", () => {
      ...
      expect(
        wrapper
          .find(".confirmation-infos .confirmation-info")
          .at(2)
          .find(".info-value.name")
          .text()
      ).toBe(props.account.name);
    });
    ...
    it("Fourth .confirmation-info should be the currency", () => {
      ...
      expect(
        wrapper
          .find(".confirmation-infos .confirmation-info")
          .at(3)
          .find(".info-value.currency")
          .text()
      ).toBe(props.account.currency.units[0].name);
    });
