...
describe('AccountApprove component', () => {
  ...
  it('should have a .header', () => {
    ...
    expect(wrapper.find('#account-creation .header')
      .length,
    ).toBe(1);
  });
  ...
  it('should have CircularProgress inside #account-creation.wrapper.loading .content', () => {
    const wrapper = shallow(<AccountApprove {...props} />);
    expect(wrapper.find('#account-creation.wrapper.loading .content')
  ...
  it('should have a .footer', () => {
    const wrapper = shallow(<AccountApprove {...props} />);
    expect(wrapper.find('#account-creation .footer')
      .length,
    ).toBe(1);
  });
  ...
  it('should contain 3 Tab with details with details, members, approvals', () => {
    const wrapper = shallow(<AccountApprove {...mainProps} />);
    expect(wrapper.find('TabList Tab').at(0).children().text()).toBe('details');
    expect(wrapper.find('TabList Tab').at(1).children().text()).toBe('members');
    expect(wrapper.find('TabList Tab').at(2).children().text()).toBe('approvals');
  });
  ...
  it('Second TabPanel should contain an AccountApproveMembers', () => {
    const wrapper = shallow(<AccountApprove {...mainProps} />);
    const AccountApproveMembers = wrapper.find('.content TabPanel').at(1).find('AccountApproveMembers');

    expect(AccountApproveMembers.prop('organization')).toEqual(mainProps.organization);
    expect(AccountApproveMembers.prop('getOrganizationMembers')).toEqual(mainProps.getOrganizationMembers);
    expect(AccountApproveMembers.prop('account')).toEqual(mainProps.account.account);
  });

  it('Third TabPanel should contain an AccountApproveApprovals', () => {
    const wrapper = shallow(<AccountApprove {...mainProps} />);
    const AccountApproveApprovals = wrapper.find('.content TabPanel').at(2).find('AccountApproveApprovals');

    expect(AccountApproveApprovals.prop('organization')).toEqual(mainProps.organization);
    expect(AccountApproveApprovals.prop('getOrganizationMembers')).toEqual(mainProps.getOrganizationMembers);
    expect(AccountApproveApprovals.prop('getOrganizationApprovers')).toEqual(mainProps.getOrganizationApprovers);
    expect(AccountApproveApprovals.prop('account')).toEqual(mainProps.account.account);
  });
