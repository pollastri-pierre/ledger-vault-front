...
describe("AccountApproveApprovals tab component", () => {
  ...
  describe("Data ready state", () => {
    ...
    it("should have a .info.approve", () => {
      const wrapper = shallow(<AccountApproveApprovals {...sProps} />);
      expect(
        wrapper.find(".account-creation-members .info.approve").length
      ).toBe(1);
    });
    ...
    it("should display all the approvers", () => {
      const wrapper = shallow(<AccountApproveApprovals {...sProps} />);
      expect(
        wrapper.find(".account-creation-members .account-member-approval")
          .length
      ).toBe(2);
    });
    ...
    it("the first one should be approved", () => {
      const wrapper = shallow(<AccountApproveApprovals {...sProps} />);
      const First = wrapper
        .find(".account-creation-members .account-member-approval")
        .at(0);

      expect(First.find(".badge-approved").find("ValidateBadge").length).toBe(
        1
      );
    });
    ...
    it("the second one should not be approved", () => {
      ...
      expect(
        Second.find(".badge-approved.not-approved").find("Question").length
      ).toBe(1);
    });
    ...
    it("should display the percentage of approvals in .approval-percentage", () => {
      ...
      expect(Percentage.find("p").text()).toBe(
        "1 collected, 1 remaining (50%)"
      );
    });
    ...
    it("should display a percentage bar", () => {
      ...
      expect(
        wrapper.find(
          ".approval-percentage .percentage-bar .percentage-bar-fill"
        ).length
      ).toBe(1);
    });
  });
