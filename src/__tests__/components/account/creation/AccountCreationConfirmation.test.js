...
describe("AccountCreationConfirmation test", () => {
  ...
  it("first .confirmation-security-item should be members", () => {
    const member = wrapper
      .find(".confirmation-security")
      .children()
      .at(0);
    expect(member.find("People").prop("className")).toBe(
      "security-icon member"
    );
  ...
  it("second .confirmation-security-item should be timelock", () => {
    const timelock = wrapper
      .find(".confirmation-security")
      .children()
      .at(1);
    expect(timelock.find("Hourglass").prop("className")).toBe(
      "security-icon timelock"
    );
  ...
  it("third .confirmation-security-item should be ratelimiter", () => {
    const ratelimiter = wrapper
      .find(".confirmation-security")
      .children()
      .at(2);
    expect(ratelimiter.find("Rates").prop("className")).toBe(
      "security-icon ratelimiter"
    );
  ...
  it("should contain a .confirmation-infos with 3 .confirmation-info", () => {
    expect(
      wrapper.find("div.confirmation-infos").find("div.confirmation-info")
        .length
    ).toBe(3);
  });
  ...
  it("first .confirmation-info should be name of account", () => {
    const first = wrapper
      .find("div.confirmation-infos")
      .find("div.confirmation-info")
      .at(0);
