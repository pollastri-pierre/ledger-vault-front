...
describe('MainCreation Test', () => {
  ...
  it('First Tab in TabList should be currency', () => {
    ...
    expect(tablist.children().at(0).prop('children')).toBe(' 1. Currency ');
  });
  ...
  it('Second Tab in TabList should be Options', () => {
    ...
    const header = tabs.find('header');
    const tablist = header.find('TabList');
    expect(tablist.children().at(1).prop('children')).toBe('2. Options');
  });
  ...
  it('Third Tab in TabList should be Security', () => {
    ...
    const header = tabs.find('header');
    const tablist = header.find('TabList');
    expect(tablist.children().at(2).prop('children')).toBe('3. Security');
  });
  ...
  it('Fourth Tab in TabList should be Confirmation', () => {
    ...
    const header = tabs.find('header');
    const tablist = header.find('TabList');
    expect(tablist.children().at(3).prop('children')).toBe('4. Confirmation');
  });
  ...
  it('Second Tab should be disabled if no currency set', () => {
    ...
    const header = tabs.find('header');
    const tablist = header.find('TabList');
    expect(tablist.children().at(1).prop('disabled')).toBe(true);
  });
  ...
  it('Second Tab should not be disabled if currency set', () => {
    ...
    const header = tabs.find('header');
    const tablist = header.find('TabList');
    expect(tablist.children().at(1).prop('disabled')).toBe(false);
  });
  ...
  it('Third Tab should be disabled if options.name not set', () => {
    ...
    const header = tabs.find('header');
    const tablist = header.find('TabList');
    expect(tablist.children().at(2).prop('disabled')).toBe(true);
  });
  ...
  it('Third Tab should not be disabled if options.name is set', () => {
    ...
    const header = tabs.find('header');
    const tablist = header.find('TabList');
    expect(tablist.children().at(2).prop('disabled')).toBe(false);
  });
  ...
  it('Fourth Tab should be disabled if account security no set', () => {
    ...
    const header = tabs.find('header');
    const tablist = header.find('TabList');
    expect(tablist.children().at(3).prop('disabled')).toBe(true);
  });
  ...
  it('second TabPanel should Contain AccountCreationOptions', () => {
    const wrapper = shallow(<MainCreation {...props} />);
    expect(wrapper.find('TabPanel').children().at(1).type()).toBe(AccountCreationOptions);
  });

  it('AccountCreationOptions should have the right props', () => {
    const wrapper = shallow(<MainCreation {...props} />);
    const options = wrapper.find('TabPanel').at(1).children().at(0);
  ...
  it('third TabPanel should Contain AccountCreationSecurity', () => {
    const wrapper = shallow(<MainCreation {...props} />);
    expect(wrapper.find('TabPanel').children().at(2).type()).toBe(AccountCreationSecurity);
  });

  it('AccountCreationSecurity should have the right props', () => {
    const wrapper = shallow(<MainCreation {...props} />);
    const securities = wrapper.find('TabPanel').at(2).children().at(0);

    expect(securities.prop('switchInternalModal')).toBe(props.switchInternalModal);
    expect(securities.prop('account')).toBe(props.account);
  });

  it('fourth TabPanel should Contain AccountCreationConfirmation', () => {
    const wrapper = shallow(<MainCreation {...props} />);
    expect(wrapper.find('TabPanel').children().at(3).type()).toBe(AccountCreationConfirmation);
  });

  it('AccountCreationConfirmation should have the right props', () => {
    const wrapper = shallow(<MainCreation {...props} />);
    const confirmation = wrapper.find('TabPanel').at(3).children().at(0);
