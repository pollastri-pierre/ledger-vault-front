...
context("Registration Shared Owners", () => {
  ...
  it("should add 3 Shared Owners", async () => {
    ...
    await cy.request("POST", DEVICE, { device_number: 7 });

    await cy.visit(Cypress.env("api_server"), {
      onBeforeLoad: win => {
        win.fetch = null;
        win.eval(polyfill);
    ...
    // Shared Owner 2
    await cy.request("POST", DEVICE, { device_number: 8 });
    cy.contains("Add shared-owner").click();
    cy.wait("@authenticate");

    // Shared Owner 3
    await cy.request("POST", DEVICE, { device_number: 9 });
